...
export default function WishlistPage() {
  ...
  const filteredItems = wishlistItems.filter(item => {
    const categoryMatch = selectedCategory === 'Все товары' || 
                         item.product.category.name === selectedCategory
    const brandMatch = selectedBrand === 'Все бренды' || 
                      item.product.brand?.name === selectedBrand
    return categoryMatch && brandMatch
  })

  // Получение уникальных категорий и брендов
  const categories = ['Все товары', ...Array.from(new Set(wishlistItems.map(item => item.product.category.name)))]
  const brands = ['Все бренды', ...Array.from(new Set(wishlistItems.map(item => item.product.brand?.name).filter(Boolean)))]
